...
name: gt-submit
version: 1.0.8
description: Submit current branch and its ancestors to Graphite, creating or updating PRs while preserving their draft or ready state
allowed-tools: Bash(git status), Bash(gt sync), Bash(gt log *), Bash(gt submit --no-edit), Bash(gt info *), Bash(git diff *), Bash(gh pr view *), Bash(gh pr edit *), Bash(gh pr diff *)
---
...
5. Run `gt submit --no-edit` to push and create/update PRs. Do not pass `--draft` or `--publish`: the default behavior preserves an existing PR's draft or ready state, and creates new PRs as drafts. Use the `/gt-publish` skill only when the user explicitly wants to mark the PR ready for review.
