...
1. Invoke the `/review-comments-and-fix` skill to fetch unresolved comments and apply fixes for any **Fix now** items
2. If any fixes were applied, invoke the `/gt-modify-submit` skill to commit and push them — so the resolved threads point at real code on the PR
3. Invoke the `/update-threads` skill to reply with verdicts and resolve each thread
